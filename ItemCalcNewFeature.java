...
public class ItemCalculator {
    ...
    private static final double SURCHARGE_RATE_MANUFACTURED = 0.02; // Additional 2% surcharge for Manufactured items
    ...
    private double calculateSalesTax(String type, double price) {
        ...
        if (type.equalsIgnoreCase("Manufactured")) {
            salesTax += (price + salesTax) * SURCHARGE_RATE_MANUFACTURED;
        }
    ...
    public void calculateTotalCost() {
        ...
        for (Item item : itemList) {
            double itemBaseTotal = item.getPrice() * item.getQuantity();
            double salesTax = calculateSalesTax(item.getType(), itemBaseTotal);
            double itemTotal = itemBaseTotal + salesTax;
            totalCost += itemTotal;
